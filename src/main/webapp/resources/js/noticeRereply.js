...
var rereplyService = (function(){
	...
	function getList(param, callback, error){
		...
		$.getJSON("/noticeRereplies/pages/" + rno + "/" + page + ".json",
			function(data){
				if (callback) {
	...
	function update(rereply, callback, error){
		...
		$.ajax({
			type : 'put',
			url : '/noticeRereplies/' + rereply.rrno,
			data : JSON.stringify(rereply),
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr){
				if (callback) {
	...
	function get(rrno, callback, error){
		$.get("/noticeRereplies/" + rrno + ".json", function(result) {
			if (callback) {
				callback(result);
				}
			}).fail(function(xhr, status, err) {
