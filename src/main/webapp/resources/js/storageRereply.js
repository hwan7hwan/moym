...
var rereplyService = (function(){
	...
	function getList(param, callback, error){
		...
		$.getJSON("/storageRereplies/pages/" + rno + "/" + page + ".json",
			function(data){
				if (callback) {
	...
	function update(rereply, callback, error){
		...
		$.ajax({
			type : 'put',
			url : '/storageRereplies/' + rereply.rrno,
			data : JSON.stringify(rereply),
			contentType : "application/json; charset=utf-8",
			success : function(result, status, xhr){
				if (callback) {
	...
	function get(rrno, callback, error){
		$.get("/storageRereplies/" + rrno + ".json", function(result) {
			if (callback) {
				callback(result);
				}
			}).fail(function(xhr, status, err) {
